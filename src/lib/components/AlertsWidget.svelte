<script lang="ts">
	import { weatherAlerts, hasActiveAlerts } from '../stores/weather.js';
	import { criticalEvents, recentEvents } from '../stores/disaster.js';
...
<div class="widget">
	<div class="widget-header">
		...
		<div class="flex items-center space-x-2">
			{#if $hasActiveAlerts || $criticalEvents.length > 0}
				<div class="status-indicator status-warning animate-pulse"></div>
				<span class="text-sm text-yellow-600">Active</span>
			{:else}
				<div class="status-indicator status-online"></div>
				<span class="text-sm text-amber-700">Normal</span>
			{/if}
		</div>
	</div>
	
	<div class="widget-content">
		{#if $hasActiveAlerts || $criticalEvents.length > 0 || $recentEvents.length > 0}
			<div class="space-y-4">
